...
export default function UploadPage() {
  ...
  const acceptFile = useCallback((f: File) => {
    ...
    if (ext !== 'glb' && ext !== 'gltf') {
      setError('Format non supporté. Veuillez utiliser un fichier .glb ou .gltf')
      setState('error')
      return
    }
    if (f.size > 500 * 1024 * 1024) {
      setError('Fichier trop volumineux. Limite : 500 MB')
      setState('error')
      return
    }
    setFile(f)
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!initRes.ok) throw new Error('Erreur lors de l\'initialisation')
      ...
      if (!confirmRes.ok) throw new Error('Erreur de confirmation')
  ...
  return (
    <div className="p-8 max-w-2xl">
      <div className="mb-8">
        <h1 className="text-2xl font-bold mb-1 text-dark-900">Nouvel upload</h1>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex: Boîtier Pompe v3 - Assemblage"
      ...
      {state === 'done' && (
        <div className="text-center py-12 bg-white border border-surface-200 shadow-sm rounded-2xl">
          ...
          <h2 className="text-xl font-semibold mb-2 text-dark-900">Upload réussi !</h2>
          <p className="text-dark-700 text-sm">Redirection vers votre projet...</p>
        </div>
      )}
    </div>
  )
